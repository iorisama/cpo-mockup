...
export const generateNextTick = (currentMachines) => {
  return currentMachines.map(machine => {
    // only slightly fluctuate values
...
export const generateRandomAnomaly = (machines) => {
  // Pick a random machine
  const randomMachine = machines[Math.floor(Math.random() * machines.length)];
  
  // Pick a random anomaly for that machine
  ...
  return {
    ...
    type: randomAnomaly.type, // 'warning' or 'error'
